...
---------
--------- Buttons
mkstr(SI.TELE_UI_BTN_SEARCH_PLAYER, "プレイヤーで検索（フィルタ）")
mkstr(SI.TELE_UI_BTN_SEARCH_ZONE, "ゾーン名で検索（フィルタ）")
...
mkstr(SI.TELE_UI_BTN_RELATED_QUESTS, "クエスト関連ゾーン")
...
mkstr(SI.TELE_UI_NO_MATCHES, "見つかりませんでした。")
...
mkstr(SI.TELE_UI_DISCOVERED_WAYSHRINES, "発見した旅の祠:")
mkstr(SI.TELE_UI_DISCOVERED_SKYSHARDS, "発見したスカイシャード:")
---------
--------- Menu
...
mkstr(SI.TELE_UI_WHISPER_PLAYER, "ささやく")
mkstr(SI.TELE_UI_JUMP_TO_HOUSE, "本邸を訪ねる")
...
mkstr(SI.TELE_UI_SET_PRIMARY_HOUSE, "本邸に設定する")
mkstr(SI.TELE_UI_TOGGLE_HOUSE_NICKNAME, "ニックネームを表示する")
mkstr(SI.TELE_UI_VIEW_MAP_ITEM, "地図を表示")
mkstr(SI.TELE_UI_BANK, "銀行:")
mkstr(SI.TELE_UI_LEAD, "手掛かり:")
mkstr(SI.TELE_UI_TOGGLE_SURVEY_MAP, "調査報告")
...
mkstr(SI.TELE_UI_TOGGLE_ALL_DELVES, "すべて表示")
mkstr(SI.TELE_UI_DAYS_LEFT, "残り日数 %d")
mkstr(SI.TELE_UI_TOGGLE_INCLUDE_BANK_MAP, "銀行を含む")
...
-- CHAT OUTPUTS
-----------------------------------------------------------------------------
mkstr(SI.TELE_CHAT_ERROR_WHILE_PORTING, "プレイヤーにテレポート出来ません")
mkstr(SI.TELE_CHAT_TO_PLAYER, "プレイヤーにテレポートしています:")
mkstr(SI.TELE_CHAT_UNLOCK_START_INFO, "自動ロック解除を開始します...")
mkstr(SI.TELE_CHAT_ERROR, "テレポート中にエラーが発生しました")
mkstr(SI.TELE_CHAT_UNLOCK_WS_SUCCESS, "自動解除が正常終了しました")
mkstr(SI.TELE_CHAT_UNLOCK_WS_COUNT_CALC, "解除された旅の祠を割り出し中 ...")
mkstr(SI.TELE_CHAT_UNLOCK_WS_COUNT_PLU, "新しい旅の祠の解除が終了しました")
mkstr(SI.TELE_CHAT_UNLOCK_WS_COUNT_SING, "新しい旅の祠が解除されました")
mkstr(SI.TELE_CHAT_UNLOCK_WS_NO_PLAYERS, "テレポート先にプレイヤーがいません")
...
mkstr(SI.TELE_CHAT_FAVORITE_ZONE_NO_FAST_TRAVEL, "ファストトラベルのオプションが見つかりません")
mkstr(SI.TELE_CHAT_NOT_IN_GROUP, "グループに所属していません")
mkstr(SI.TELE_CHAT_PORTING_NOT_POSSIBLE, "現在ファストトラベルが出来ません")
mkstr(SI.TELE_CHAT_PORT_TO_OWN_HOUSE, "持ち家にテレポートしています:")
...
mkstr(SI.TELE_CHAT_TO_TRACKED_QUEST, "フォーカスしているクエストへの移動：")
...
mkstr(SI.TELE_SETTINGS_DISABLE_DIALOG_TOOLTIP, "自動解除機能を使用した時の確認メッセージを表示しません。")
...
mkstr(SI.TELE_SETTINGS_HOUSE_NICKNAMES, "家のニックネームを表示する")
mkstr(SI.TELE_SETTINGS_HOUSE_NICKNAMES_TOOLTIP, "家の名前の代わりに、ニックネーム（変更可能です）を表示します。")
...
mkstr(SI.TELE_SETTINGS_INFO_CHARACTER_DEPENDING, "|ca20000この項目はキャラクター毎に設定されます(グローバル設定ではありません)！|r")
...
mkstr(SI.TELE_KEYBINDING_TOGGLE_MAIN_DELVES, "現在のゾーンの洞窟")
...
mkstr(SI.TELE_DIALOG_AUTO_UNLOCK_BODY, "BeamMeUpは現在のゾーンにいる他のプレイヤー達への不完全なファストトラベルを開始します。もし他のプレイヤーが未開放の旅の祠付近にいた場合その旅の祠を使えるようになりますので、将来に役立つでしょう。ファストトラベルを完了する必要ありません。移動してテレポート処理を中断することができます。 \n[注意] 実際に訪れるまでは「完了した地図」に反映されません。")
...
mkstr(SI.TELE_DIALOG_REFUSE_AUTO_UNLOCK_TITLE, "旅の祠の解除はもう必要ありません。")
...
mkstr(SI.TELE_DIALOG_REFUSE_AUTO_UNLOCK_TITLE2, "現在、旅の祠の解除は実行できません。")
