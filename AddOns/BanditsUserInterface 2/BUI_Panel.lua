...
local Disabled={
	...
	Assistans		=function()
					return not(
					IsCollectibleUnlocked(267) or IsCollectibleUnlocked(6376) or IsCollectibleUnlocked(8994) or	--Banker
					IsCollectibleUnlocked(301) or IsCollectibleUnlocked(6378) or IsCollectibleUnlocked(8995) or	--Trader
					IsCollectibleUnlocked(300)	--Smuggler
					)
					end,
...
--]]
	...
	func	=function()
			local id=IsCollectibleUnlocked(8994) and 8994 or IsCollectibleUnlocked(6376) and 6376 or 267
			UseCollectible(id)
			SCENE_MANAGER:SetInUIMode()
		end,
	tooltip	=GetString(SI_INTERACT_OPTION_BANK),
	var		="Assistans",
	disabled	=function()return not IsCollectibleUnlocked(267) and not IsCollectibleUnlocked(6376) end
	},
	{	--Trader
	icon		="/esoui/art/mail/gamepad/gp_mailmenu_attachitem.dds",
	func	=function()
			local id=IsCollectibleUnlocked(8995) and 8995 or IsCollectibleUnlocked(6378) and 6378 or 301
			UseCollectible(id)
			SCENE_MANAGER:SetInUIMode()
		end,
	tooltip	=GetString(SI_GAMEPAD_GUILD_KIOSK_TRADER_HEADER),
	var		="Assistans",
	disabled	=function()return not IsCollectibleUnlocked(301) and not IsCollectibleUnlocked(6378) end
	},
	{	--Smuggler
	icon		="/esoui/art/icons/mapkey/mapkey_fence.dds",
	func		=function()UseCollectible(300) SCENE_MANAGER:SetInUIMode()end,
	tooltip	="Smuggler",
	var		="Assistans",
	disabled	=function()return not IsCollectibleUnlocked(300) end
	},
--[[
	{	--Jubilee event
...
local function UI_Init()
	...
	for _,data in pairs(PanelContent) do
		...
		if data.divider and last~="divider" then
			...
			div=div+1
			last="divider"
		elseif BUI.Vars.SidePanel[data.var] and not disabled then
			i=i+1
			local button=_G["BUI_PanelButton"..i] or WINDOW_MANAGER:CreateControl("$(parent)Button"..i, control, CT_TEXTURE)
...
local function Menu_Init()
	...
	for _,var in pairs(Defaults) do
		...
		icon		=Icons[var],
		name		="Panel"..var,
		getFunc	=function() return BUI.Vars.SidePanel[var] end,
		setFunc	=function(value) BUI.Vars.SidePanel[var]=value UI_Init() end,
