...
local function Frame_Player_UI()	--UI init
	...
	local anchor=BUI.Vars.Theme==7 and {BOTTOMRIGHT,CENTER,BanditsUI:GetWidth()/6-5,BanditsUI:GetHeight()/6,BanditsUI} or {BOTTOMRIGHT,TOPRIGHT,-10,0}
	local bar	=BUI.UI.Control("BUI_PlayerFrame_Dots",		health,	(BUI.Vars.Theme==7 and {16,16*5+4} or {16*5+4,16}),	anchor,	BUI.Vars.Theme~=7)
	bar:SetDrawLayer(2)
	for i=1,5 do
		bar[i]=BUI.UI.Texture("BUI_PlayerFrame_Dots"..i,	bar,		{16,16},		(BUI.Vars.Theme==7 and {BOTTOM,BOTTOM,0,-(i-1)*16-3} or {LEFT,LEFT,(i-1)*16+3,0}),	dot_off, false)
...
--	member.bg		=BUI.UI.Backdrop("BUI_RaidFrame"..i.."_Bg",	member,	{w-6*s+fs*1.2,h-2*s},		{TOPLEFT,TOPLEFT,-fs*1.2+2*s,0},{.3,.3,.2,.7}, {0,0,0,0}, nil, not comp) member.bg:SetDrawLayer(0)
	...
	health.current	=BUI.UI.Label(	"BUI_RaidFrame"..i.."_Current",health,	{w*2/3,hs},		(comp and {RIGHT,RIGHT,-fs*1.3,0} or {LEFT,LEFT,8,0}),		BUI.UI.Font(BUI.Vars.FrameFont1,fs,true), nil, {comp and 2 or 0,1}, '0.0k', false)
	health.current:SetDrawLayer(3)
...
--		if BUI.Vars.DeveloperMode and BUI.BossFight then d(BUI.TimeStamp().."|cEE2222Boss fight end|r") end
		...
		for i=1, 3 do BUI.Frame["boss"..i]:SetHidden(true) end
...
function BUI.Frames.Bosses_Init(phase)
	...
	for i=1, 3 do
		local unitTag="boss"..i
		if DoesUnitExist(unitTag) then
			if unitTag=="boss1" then n_bosses=1 elseif unitTag=="boss2" then n_bosses=2 elseif unitTag=="boss3" then n_bosses=3 end
