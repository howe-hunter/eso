...
--			ability.icon:SetColor(unpack(BUI.PlayerBuffs[i].Color))
	...
	for i=math.min(number+1,15), 15 do _G["BUI_BuffsP"..i]:SetHidden(true) end
	--Passives
...
--				ability.icon:SetColor(unpack(BUI.PassiveBuffs[i].Color))
		...
		for i=math.min(number+1,12), 12 do _G["BUI_BuffsPas"..i]:SetHidden(true) end
...
--				if BUI.Buffs.Horn[abilityId] then BUI.Buffs.HornActive=true if BUI.OnScreen.Message[3] then BUI.OnScreen.Message[3]={["message"]="",["time"]=0} BUI.OnScreen.Update() end end
			...
			--Buffs module -------------------------
			if BUI.Vars.PlayerBuffs and abilityId~=14890 and not (BUI.Vars.EnableBlackList and (BUI.Vars.BuffsBlackList[abilityId] or BUI.Vars.BuffsBlackList[buffName])) then
				--Vampirism stage
...
--							Plate=zo_strformat("[<<1>>]\n<<2>>(<<3>>)",abilityId,tostring((effectType==1)),tostring(effectType)),
	...
	--Food
	if have_food then
		if have_food<10000 then
			if BUI.Vars.NotificationFood then BUI.OnScreen.Notification(0,BUI.Loc("Food"),nil,have_food) end
		end
