local QUICKSLOTMAP		={12,11,10,9,16,15,14,13}
local QUICKSLOTS			={[12]=1,[11]=2,[10]=3,[9]=4,[16]=5,[15]=6,[14]=7,[13]=8}
local DefaultQuickSlot		=ZO_ActionBar_GetButton(ACTION_BAR_FIRST_UTILITY_BAR_SLOT+1)
...
local function OnSlotAbilityUsed(_,slot)
	...
	local remain,duration=GetSlotCooldownInfo(_slot)
	EffectSlot=slot
	d(remain,duration)
	AddEffect()
end
...
local function OnSlotChanged()
	...
	if i<=BUI.Vars.QuickSlotsShow then
		BUI_QuickSlots[i].status:SetHidden(false)
		BUI_QuickSlots[i].count:SetText(GetSlotItemCount(slot))
	end
...
local function SlotsUpdate()
	for i=1,8 do
		...
		if slot then
			local texture=GetSlotTexture(slot.slot)
			slot.icon:SetTexture(texture)
			slot.icon:SetHidden(texture=="")
			slot.count:SetText(GetSlotItemCount(slot.slot))
		end
	end
end
...
--	local parent	=slots==9 and ZO_SharedRightPanelBackground or ZO_ActionBar1
	...
	if BUI.Vars.QuickSlots then
		for i=1,math.min(slots,8) do
			...
			local texture=GetSlotTexture(QUICKSLOTMAP[i])
			...
			ui[i].count		=BUI.UI.Label("BUI_QuickSlot"..i.."Count", ui[i].icon, {h-space*2,h*.4}, {BOTTOMLEFT,BOTTOMLEFT,0,-space}, BUI.UI.Font("esobold",h*.4,true), nil, {2,1}, GetSlotItemCount(QUICKSLOTMAP[i]))
...
--		EVENT_MANAGER:RegisterForEvent("BUI_QS_Event", EVENT_ACTION_SLOT_UPDATED,		OnActionSlotsUpdate)
		...
		for _,frame in pairs({ZO_QuickSlot,ZO_PlayerInventory}) do
