...
--[[	--Quick slots
	for i=9,17 do
		local name="BUI_QuickSlotButton"..i
		local frame=_G[name]
		if frame then
			local button=frame:GetNamedChild("Button")
			button:SetNormalTexture("") button:SetPressedTexture("")
			local edge=BUI.UI.Texture(name.."Edge", frame, "inherit", {TOPLEFT,TOPLEFT,0,0}, normal, false)
			if color then edge:SetColor(unpack(color)) end edge:SetDrawLayer(2)
		end
	end

	ActionButton.ApplyStyle=function()end

	ZO_ActionSlot_SetupSlot=function(iconControl, buttonControl, icon, normalFrame, downFrame, cooldownIconControl)
		iconControl:SetHidden(false) iconControl:SetTexture(icon)
		if cooldownIconControl then cooldownIconControl:SetTexture(icon) end
		buttonControl:SetNormalTexture("") buttonControl:SetPressedTexture("")
	end

	ZO_ActionSlot_ClearSlot=function(iconControl, buttonControl, normalFrame, downFrame, cooldownIconControl)
		iconControl:SetHidden(true)
		if cooldownIconControl then cooldownIconControl:SetHidden(true) end
		buttonControl:SetNormalTexture("") buttonControl:SetPressedTexture("")
	end
--]]
end
...
--		for i,data in pairs(textures) do RedirectTexture("/esoui/art/"..data[1].."/"..data[2],"/BanditsUserInterface/textures/theme/"..data[2]) end
	...
	if BUI.Vars.Theme==6 then
		if BUI_Theme_Pony then
			...
			BUI.UI.Statusbar("BUI_Theme_Pony",ActionButton9,{128*scale,128*scale},{BOTTOMRIGHT,BOTTOMRIGHT,0,0},{1,1,1,1},"/BanditsUserInterface/textures/pink_pony.dds",false)
		end
