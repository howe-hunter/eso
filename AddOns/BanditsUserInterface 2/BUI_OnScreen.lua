...
local function Glyphs_Init()
	local size	=32
	local anchor=(BUI.Vars.BUI_Glyphs) and BUI.Vars.BUI_Glyphs or {BOTTOMRIGHT,BOTTOMLEFT,0,0,ZO_ActionBar_GetButton(ACTION_BAR_FIRST_UTILITY_BAR_SLOT+1).slot}
...
--				if BUI.Vars.DeveloperMode then d(BUI.TimeStamp()..'Notification "'..mes.message..'" expired') end
...
local function CombatEventNotification(result,targetName,hitValue,sourceUnitId,targetUnitId,abilityId)
	...
	if alert then
		...
		if type(alert)=="number" then
			if alert==200 then	--RL
				if BUI.Vars.DeveloperMode or BUI.Player.isLeader then
					BUI.OnScreen.NotificationSecondary(abilityId,GetAbilityName(abilityId),BUI.Vars.NotificationSound_1,hitValue,sourceUnitId)
				end
				return
		...
		if string.gsub(targetName,"%^%w+","")==BUI.Player.name then
			BUI.OnScreen.Notification(abilityId,string.gsub(GetAbilityName(abilityId),"%^%w+",""),BUI.Vars.NotificationSound_1,hitValue,sourceUnitId,(phase and "|cFF2222You!" or nil))
		elseif phase then
			targetName=BUI.GroupMembers[targetUnitId]
			if targetName or phase=="Primary" then
				BUI.OnScreen.Notification(abilityId,string.gsub(GetAbilityName(abilityId),"%^%w+",""),BUI.Vars.NotificationSound_1,hitValue,sourceUnitId,BUI.Group[targetName] and BUI.Group[targetName].accname or targetName)
			else
				BUI.OnScreen.NotificationSecondary(abilityId,string.gsub(GetAbilityName(abilityId),"%^%w+",""),BUI.Vars.NotificationSound_1,hitValue,sourceUnitId)
			end
		end
	end
end
