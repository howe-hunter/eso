...
function IIfA:RescanHouse(houseCollectibleId)
  ...
  task:Call(function()
    ...
    for itemLink, itemCount in pairs(placedFurniture) do
      ...
      IIfA:EvalBagItem(houseCollectibleId, tonumber(IIfA_GetItemID(itemLink)), false, itemCount, itemLink, itemName, houseCollectibleId)
    end
  end)
...
function IIfA:GetItemKey(itemLink)
  ...
  if CanItemLinkBeVirtual(itemLink) then
    ...
    if (itemType == ITEMTYPE_TOOL and subType == SPECIALIZED_ITEMTYPE_TOOL) or
      ...
      (itemType == ITEMTYPE_CONTAINER and subType == SPECIALIZED_ITEMTYPE_CONTAINER) then
      -- 4-6-19 AM - runeboxes appear to have level info in them, ignored now and used item id instead
      return IIfA_GetItemID(itemLink)
    end
  end
  return itemLink
...
function IIfA:AddOrRemoveFurnitureItem(itemLink, itemCount, houseCollectibleId, fromInitialize)
  ...
  local itemName = zo_strformat(SI_TOOLTIP_ITEM_NAME, GetItemLinkName(itemLink))
  IIfA:EvalBagItem(houseCollectibleId, IIfA_GetItemID(itemLink), false, itemCount, itemLink, itemName, houseCollectibleId)
end
