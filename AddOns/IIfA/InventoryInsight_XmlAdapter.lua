...
function IIfA:GuiOnFilterButton(control, mouseButton, filterGroup, filterTypes, filterTypeNames)
  ...
  if b_isMain then
    ...
    if filterGroup ~= GetString(IIFA_LOCATION_NAME_ALL) then
  ...
  if filterTypeNames ~= nil then
    ...
    local entry = comboBox:CreateItemEntry(GetString(IIFA_LOCATION_NAME_ALL), SetSubSubFilters)
    entry.subFiltTypes = IIfA.filterTypes
    comboBox:AddItem(entry)
    if IIfA.filterGroup == "Body" or IIfA.filterGroup == "Specialized" then
    ...
    comboBox:SetSelectedItem(GetString(IIFA_LOCATION_NAME_ALL))
...
function IIfA:onResizeStart()
  EVENT_MANAGER:RegisterForUpdate(IIfA.name .. "OnWindowResize", 50, function()
    IIfA:GuiResizeScroll()
    if not IIfA:GetSceneSettings().docked then
      IIfA:UpdateInventoryScroll()
    end
  end)
end
