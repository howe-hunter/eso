...
function IIfA:makeFont(face, size, effect)
  local fontFace, fontSize, fontEffect = face, size, effect
  if not fontFace then fontFace = IIfA.defaults.TooltipFontFace end
  if not fontSize then fontSize = IIfA.defaults.TooltipFontSize end
  if not fontEffect then fontEffect = IIfA.defaults.TooltipFontEffect end

  self.TooltipFont = fontFace .. "|" .. fontSize
  if fontEffect ~= nil and fontEffect ~= "none" and fontEffect ~= "" then
    self.TooltipFont = self.TooltipFont .. "|" .. fontEffect
  end
...
function IIfA:SetTooltipFont(fontFull, fontFace, fontSize, fontEffect)
  --[[TODO how is font valid here when it is considered a global, but to what
  and it is not defined anywhere as a global var ]]--
  ...
  local currentFontFace = settings.TooltipFontFace
  local currentFontSize = settings.TooltipFontSize
  local currentFontEffect = settings.TooltipFontEffect
  local rebuild = false
  ...
  if currentFontFace ~= settings.TooltipFontFace then
    rebuild = true
  end
  if currentFontSize ~= settings.TooltipFontSize then
    rebuild = true
  end
  if currentFontEffect ~= settings.TooltipFontEffect then
    rebuild = true
  end
